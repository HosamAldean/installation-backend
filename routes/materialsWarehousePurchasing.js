        ...
        for (const line of items) {
            ...
            if (qtyAccepted > 0) {
                const ledgerRow = await postLedgerMovement({
                    ...
                    // poItem's own color (null for a mill-finish shortfall
                ...
                // whatever physically arrived into that SAME draft job's
                // qtySent (handles a shortfall PO received across more than
                // one goods receipt without creating a duplicate job per
                // receipt). Still not a real physical movement (no ledger
                // post, no vendor, sentDate null) -- a storekeeper still has
                // to pick a vendor and explicitly confirm/send it via
                // POST /external-processing/:id/confirm-send before
                // anything actually leaves the store.
                if (poItem?.targetColor) {
                    const existingJob = await MatWhExternalProcessing.findOne({
                        where: { sourcePurchaseOrderItemId: poItem.id, status: 'draft' },
                        transaction: t,
                    });
                    if (existingJob) {
                        await existingJob.update(
                            { qtySent: Number(existingJob.qtySent) + qtyAccepted },
                            { transaction: t },
                        );
                    } else {
                        // Defensive fallback only -- covers a shortfall PO
                        ...
                        await MatWhExternalProcessing.create({
                            ...
                            sourcePurchaseOrderItemId: poItem.id,
                            isAutoGenerated: true, status: 'draft',
                            qtySent: qtyAccepted,
                            processVendorId: null, sentBy: null, sentDate: null,
                        }, { transaction: t });
                    }
                }
            }
