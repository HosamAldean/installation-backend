    ...
    try {
        ...
        for (const line of items) {
            ...
            if (qtyAccepted > 0) {
                ...
                // A mill-finish shortfall line remembers what color it
                // needs to become once coated (targetColor, set only by
                // the auto-routing logic in matWhReservations.js) -- its
                // receipt auto-creates a DRAFT coating job, not yet a real
                // physical movement (no ledger post, no vendor, sentDate
                // null). A storekeeper still has to pick a vendor and
                // explicitly confirm/send it via POST /external-
                // processing/:id/confirm-send before anything actually
                // leaves the store -- same "auto-raised, explicitly
                // confirmed at every later stage" shape the shortfall's own
                // auto-PO already has.
