...
async function confirmOneLine(line, header, confirmedBy, t) {
    ...
    if (qtyShortfall > 0) {
        ...
        // The coating/Mix request is raised right now, alongside the
        // shortfall PO -- not deferred until the mill-finish material is
        // actually received (the goods-receipt route used to be the only
        // place this got created; now it just tops this same row's
        // qtySent up as receipts come in, see POST /goods-receipts).
        // qtySent starts at 0 -- there's no material yet, only the
        // knowledge that this line will need coating once there is. See
    ...
    try {
        ...
        for (const group of shortfallGroups.values()) {
            ...
            for (const { reservationItem, qty, needsCoating } of group.lines) {
                ...
                // Same "raise the coating request now, top it up as
